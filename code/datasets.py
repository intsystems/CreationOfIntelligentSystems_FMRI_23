...
class fMRIDataset(Dataset):
    def __init__(self, seq_len: int = 1, total_len=641, sub_dir = 'ds003688-download'):
        super().__init__()
        self.data_dir = os.path.join(sub_dir, 'fMRI_to_tensors')
...
class fMRIVideoDataset(Dataset):
    def __init__(self, train=True, seq_len=1, sub_dir =  'ds003688-download',video_path='Film stimulus.mp4', skip_frames=1, total_len=641):
        super().__init__()
        assert seq_len % 5 == 0
        self.seq_len = seq_len
        self.total_len = total_len
